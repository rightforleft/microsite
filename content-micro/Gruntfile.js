module.exports = function(grunt) {
	...
	// Default task(s)
	grunt.registerTask('default', ['cleanup','htmlmin:dist','concat:css', 'concat:cssResponsive', 'cssmin', 'copy:css', 'copy:fancyboxImages', 'copy:fonts','concat:jsFancyBox', 'concat:jsOffCanvasMenu', 'copy:js', 'copy:favicons','copy:images','uglify']);
	...
	grunt.registerTask('js', ['concat:jsFancyBox', 'concat:jsOffCanvasMenu','concat:jsPost','copy:js', 'uglify']);
	...
	grunt.initConfig({
		...
		'concat': {
			css: {
				src: [
					...
					'src/plugins/off-canvas-menu/css/menu_elastic.css',
			...
			jsPost: {
				...
				dest: 'build/js/seismicPost.min.js'
			}
		...
		'uglify': {
			...
			seismicPost: {
				files: {
					'dist/js/seismicPost.min.js': ['build/js/seismicPost.min.js']
				}
			}
		},
		...
		'copy': {
			js: {
				files: [
					...
					{expand: false, src: ['src/js/jquery.min.js'], dest: 'dist/js/jquery.min.js'},
